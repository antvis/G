...
describe('Polygon', function() {
  ...
  it('init attr', function() {
    ...
    expect(polygon.attr('lineWidth')).to.be(1);
  ...
  it('points', function() {
    polygon.attr('points', []);
    expect(polygon.attr('points').length).to.be(0);
    expect(polygon.getBBox()).to.be(null);
    polygon.attr('points', [[30, 30], [40, 20], [30, 50], [60, 100]]);
    expect(polygon.attr('points').length).to.be(4);
    var box = polygon.getBBox();
    expect(box.minX).to.be(29.5);
    expect(box.maxX).to.be(60.5);
    expect(box.minY).to.be(19.5);
    expect(box.maxY).to.be(100.5);

    var polygon1 = new G.Polygon({
      attrs: {
        points: [[58, 60], [80, 190], [32, 53], [45, 32]]
      }
    });
    var box = polygon1.getBBox();
    expect(box.minX).to.be(31.5);
    expect(box.minY).to.be(31.5);
    expect(box.maxX).to.be(80.5);
    expect(box.maxY).to.be(190.5);
  });

  it('lineWidth', function() {
    expect(polygon.attr('lineWidth')).to.be(1);
    polygon.attr('lineWidth', 2);
    expect(polygon.attr('lineWidth')).to.be(2);
    var box = polygon.getBBox();
    expect(box.minX).to.be(29);
    expect(box.maxX).to.be(61);
    expect(box.minY).to.be(19);
    expect(box.maxY).to.be(101);
    ...
    var box = polygon1.getBBox();
    expect(box.minX).to.be(31);
    expect(box.minY).to.be(31);
    expect(box.maxX).to.be(81);
    expect(box.maxY).to.be(191);
  });

  it('stroke', function() {
    polygon.attr('stroke', 'l (90) 0:#f0ff0f 1:#ff0e0d');
    expect(polygon.attr('stroke')).to.be('l (90) 0:#f0ff0f 1:#ff0e0d');
    canvas.add(polygon);
    canvas.draw();
  });
  ...
  it('fill', function() {
    ...
    expect(polygon.attr('fill')).to.be('r (0.3, 0.2, 0) 0:#edda2f 1:#23edfa');
    canvas.draw();
  });
