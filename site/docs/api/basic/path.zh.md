...
如下 [示例](/examples/shape/path#path) 定义了一条直线，在局部坐标系下从 `[100, 100]` 到 `[200, 200]`：
...
默认锚点定义的位置为包围盒左上角顶点，可以通过 [anchor](/api/basic/display-object#anchor) 改变。
...
[path](/api/basic/path#path) 属性的别名，与 SVG 中的 `<path>` 命名保持一致。

### markerStart

由于 Path 可通过 `Z` 命令闭合，因此对于 “起始点” 的定义在两种情况下有差别：

- 如果未闭合，可以参考 [Polyline](/api/basic/polyline) 的 [markerStart](/api/basic/polyline#markerstart) 属性。
- 如果已闭合，可以参考 [Polygon](/api/basic/polygon) 的 [markerStart](/api/basic/polygon#markerstart) 属性。
...
在该[示例](/examples/shape/path#path)中，我们在 Path 的起始点上放置了一个箭头：
...
- 如果未闭合，可以参考 [Polyline](/api/basic/polyline) 的 [markerEnd](/api/basic/polyline#markerend) 属性。
- 如果已闭合，可以参考 [Polygon](/api/basic/polygon) 的 [markerEnd](/api/basic/polygon#markerend) 属性。

在该[示例](/examples/shape/path#path)中，我们在多边形的终止点上放置了一个图片：
...
例如下图中在路径上除首尾的每个顶点上都放置了一个 [Circle](/api/basic/circle)：
...
可以参考 [Polyline](/api/basic/polyline) 的 [markerStartOffset](/api/basic/polyline#markerstartoffset) 属性。marker 会沿路径中第一段的切线方向移动，同时主体路径也会进行相应延长或缩短。需要注意的是主体路径的伸缩距离也是有限的，当超过了第一段的长度，会产生“拐弯”的效果，如下图所示：
...
可以参考 [Polyline](/api/basic/polyline) 的 [markerEndOffset](/api/basic/polyline#markerendoffset) 属性。marker 会沿路径中最后一段的切线方向移动，同时主体路径也会进行相应延长或缩短。
...
另外在原路径上应用的，在局部坐标系下的变换也会应用到返回的点上。例如在该[示例](/examples/shape/path#path)中，路径本身经过了平移和缩放：
