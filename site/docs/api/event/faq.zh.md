...
事件系统只会响应 Canvas 画布范围之内的事件，例如监听了 mousemove 时，在画布之外的其他页面区域移动并不会触发该事件处理器。当拾取到画布空白区域（未命中任何可见图形）时，事件对象的 [target](/api/event/event-object#target) 属性会返回 [Document](/api/builtin-objects/document)：
...
一些内置事件有触发顺序，例如 click 事件会在 pointerdown 和 pointerup 触发之后。在这个过程中，有可能出现 pointerdown 和 pointerup 事件 target 不一致的情况。例如在一个图形上按下鼠标，移动到另一个图形上再抬起鼠标，此时我们会在这两个 target 共同的祖先节点上（例如场景图的根节点 [document.documentElement](/api/canvas/intro#%E5%85%A5%E5%8F%A3%E4%B8%8E%E6%A0%B9%E8%8A%82%E7%82%B9)）触发 click 事件。

可以在[这个例子](/examples/event/event-others/#delegate)中尝试。

## 在 Chrome 中禁止页面默认滚动行为

有时我们需要禁止掉页面默认的滚动行为，例如实现缩放类的需求时。禁用默认行为可以使用 [preventDefault](/api/event/event-object#preventdefault)，但以下代码在 Chrome 中执行并不会生效，页面依然可以滚动：
...
现在回到我们的问题，如果用户确实需要禁止默认滚动行为，可以在画布的 DOM 节点上手动添加一个非 Passive 的事件处理器，[g-plugin-control](http://g-next.antv.vision/zh/plugins/control) 插件就是这么做的。如何获取画布的 DOM 节点可以使用 [getDomElement](/api/canvas/built-in-objects#getcontextservice)：
...
有时我们想禁用掉浏览器默认的右键菜单，此时可以在 [contextmenu](https://developer.mozilla.org/zh-CN/docs/Web/API/Element/contextmenu_event) 事件处理函数中通过 `preventDefault()` 方法禁用默认行为。如何获取画布的 DOM 节点可以使用 [getDomElement](/api/canvas/built-in-objects#getcontextservice)：
...
可以通过 click/mouseenter/mouseleave 等事件实现焦点相关功能。[示例](/examples/event/picking/#circle)

### 鼠标双击事件

由于需要尽可能兼容 PC 和移动端事件，我们并没有监听原生的 [dblclick](https://developer.mozilla.org/zh-CN/docs/Web/API/Element/dblclick_event) 事件，而是通过监听 pointerdown 与 pointerup，将一定时间间隔（200ms）内的点击次数记录在 [detail](/api/event/event-object#detail) 属性中，这样就可以区分单击与双击：
...
在旧版中支持以下在事件名中表示委托的写法，格式为 `[被委托图形 name]:[事件名]`，[示例](/examples/event/event-others/#deprecated-delegate)：
...
在 [g-plugin-a11y](/plugins/a11y) 中，我们监听了键盘事件用于导航。
