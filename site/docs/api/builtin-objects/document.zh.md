...
- 它有指向 `window` 的引用 [defaultView](/api/builtin-objects/document#defaultview)
- 通过 [documentElement](/api/builtin-objects/document#documentelement) 访问 `<html>` 元素
- 可以通过一系列方法查询节点，例如 [getElementById](/api/builtin-objects/document#getelementbyid)
- 通过 [createElement](/api/builtin-objects/document#createelement) 创建元素
...
实现了 [Node.nodeName](/api/builtin-objects/node#nodename)，返回 `'document'`，在事件处理器中可用来快速判断 target，例如点击了画布的空白区域时：
...
返回场景图中的根节点，在创建画布时会默认使用 [Group](/api/basic/group) 创建一个：
...
由于继承自 [Node](/api/builtin-objects/node)，因此显然拥有了事件绑定能力：
...
虽然继承了 [Node](/api/builtin-objects/node)，但在 Document 上无法调用一些节点操作方法，正如在浏览器中调用 `document.appendChild` 会返回如下错误一样：
...
该方法接受一组 `x, y` 坐标（在 [Canvas 坐标系](/api/canvas/coordinates#canvas)下，如果想使用其他坐标系下的坐标，请使用[转换方法](/api/canvas/coordinates#转换方法)）为参数，返回拾取结果。

在下面的[例子](/examples/canvas/canvas-basic/#element-from-point)中，我们在 [Canvas 坐标系](/api/canvas/coordinates#canvas)下 `[100, 100]` 放置了一个半径为 `100` 的 [Circle](/api/basic/circle)，在红点处拾取时会返回它：
...
3. 拾取判定遵循以下规则：
    ...
    2. 图形的 [interactive](/api/basic/display-object#interactive) 属性**会影响**拾取。不可交互图形无法拾取。
    3. 图形的 [visibility](/api/basic/display-object#visibility) 属性**会影响**拾取。不可见图形无法拾取。
    4. 图形的 [opacity](/api/basic/display-object#opacity) 属性**不会影响**拾取。即使图形完全透明，依然也会被拾取到。

<https://developer.mozilla.org/en-US/docs/Web/API/Document/elementFromPoint>

### elementsFromPoint

当目标点上有多个图形堆叠时，该方法会按照 [z-index](/api/basic/display-object#zindex) 排序后返回它们，返回结果中的第一个元素为最顶层的图形。

该方法同样接受一组 `x, y` 坐标作为参数。

在下面的[例子](/examples/canvas/canvas-basic/#element-from-point)中，circle2 在 circle1 之上，因此在重叠区域进行拾取两者都会出现在结果数组中，并且 circle2 在前：
...
1. 该返回结果和事件对象上的 [composedPath()](/api/event/event-object#composedpath) 的差别是，后者会在返回数组中追加 [Document](/api/builtin-objects/document) 和 [Canvas](/api/canvas/intro) 对象，而前者只到 [画布根节点](/api/canvas/built-in-objects#documentdocumentelement) 为止。
2. 超出画布视口范围返回空数组。
...
1. 会考虑 [visibility](/api/basic/display-object#visibility) 和 [pointer-events](/api/basic/display-object#pointerevents) 属性
2. 无需考虑 WebGL / WebGPU 这样基于 GPU 的拾取实现，为同步方法
3. 返回的元素数组按实际渲染次序排序

### elementFromPointSync

[elementFromPoint](/api/builtin-objects/document#elementfrompoint) 的同步版本，值得注意的是，并不是所有[渲染器](/api/renderer/intro)都会实现该方法，目前仅有 [g-canvas](/api/renderer/canvas)，[g-svg](/api/renderer/svg) 和 [g-canvaskit](/api/renderer/canvaskit) 提供了对应实现：
...
[elementsFromPoint](/api/builtin-objects/document#elementsfrompoint) 的同步版本，值得注意的是，并不是所有[渲染器](/api/renderer/intro)都会实现该方法，目前仅有 [g-canvas](/api/renderer/canvas)，[g-svg](/api/renderer/svg) 和 [g-canvaskit](/api/renderer/canvaskit) 提供了对应实现：
