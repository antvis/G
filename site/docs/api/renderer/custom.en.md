...
In [Renderer Introduction](/en/docs/api/renderer/renderer), we learned that a renderer consists of a rendering context and a set of plugins that can dynamically extend the capabilities of the renderer at runtime.
...
In this method, we can get the parameters passed by the user when creating [Canvas](/en/docs/api/renderer/canvas) by injection, such as [devicePixelRatio](/en/docs/api/canvas#devicepixelratio).
...
-   [g-canvas](/en/docs/api/renderer/canvas) will return `CanvasRenderingContext2D`
-   [g-svg](/en/docs/api/renderer/svg) will return `SVGElement`
-   [g-webgl](/en/docs/api/renderer/webgl) will return a complex object `WebGLRenderingContext`
...
Different rendering contexts naturally have different difficulties to implement, for example, native [toDataURL](https://developer.mozilla.org/zh-CN/docs/Web/API/) can be used in [g-canvas](/en/docs/api/renderer/canvas) HTMLCanvasElement/toDataURL) method.
...
However, [g-svg](/en/docs/api/renderer/svg) is much more complicated to implement and requires the [XMLSerializer](https://developer.mozilla.org/zh-CN/docs/Web/API/XMLSerializer) serialization capabilities.
...
The situation is more complicated in [g-webgl](/en/docs/api/renderer/webgl), which even requires the use of asynchronous methods.
