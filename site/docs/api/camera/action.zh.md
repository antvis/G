...
很自然的，根据相机类型的不同，同一个摄像机动作对应的实现也不同。 我们以 [dolly](/api/camera/action#dolly) 动作为例，同样都是一个向前向后移动摄像机位置的动作，对于 [Orbiting](/api/camera/intro#orbiting) / [Exploring](/api/camera/intro#exploring) 模式视点不变，而在 [Tracking](/api/camera/intro#tracking) 模式下视点是需要调整的。
...
在该[示例](/examples/camera/camera-action/#action) 中，下面的操作将导致原本处于视点处的物体展示在左上角：

```ts
camera.pan(200, 200);
```

在 [g-plugin-control](/plugins/control) 中我们响应鼠标平移事件，调用该方法：

<img src="https://gw.alipayobjects.com/mdn/rms_6ae20b/afts/img/A*QjQQRLA3w8sAAAAAAAAAAAAAARQnAQ">

## dolly

沿 n 轴移动相机。固定视点，改变相机位置从而改变视距。会保持视距在 [minDistance](/api/camera/params#setmindistance) 和 [maxDistance](/api/camera/params#setmaxdistance) 之间。
...
在透视投影中效果如下，在 [g-plugin-control](/plugins/control) 中我们响应鼠标滚轮事件，调用该方法：
