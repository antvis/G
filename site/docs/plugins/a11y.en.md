...
In this [example](/en/examples/plugins/a11y/#a11y-text-extractor), we enable `enableExtractingText` to use the above functionality.
...
In the implementation we have added DOM elements inside the [canvas container](/en/api/canvas/options#container) for real-time synchronization with the visible text in the canvas.
