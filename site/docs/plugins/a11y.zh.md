...
在一些渲染器（例如 [g-canvas](/api/renderer/canvas) / [g-webgl](/api/renderer/webgl) / [g-canvaskit](/api/renderer/canvaskit)）中，当文本被绘制后便无法使用浏览器自带的搜索功能（Command + F）定位匹配，同样对于 SEO 也不友好。

在该[示例](/examples/plugins/a11y/#a11y-text-extractor)中，我们开启 `enableExtractingText` 后便可以使用以上功能：
...
在实现中我们在[画布容器](/api/canvas/options#container)内添加了 DOM 元素，用于与画布中的可见文本实时同步：
...
- 使用 [g-svg](/api/renderer/svg) 渲染时，由于 SVG 天然支持 `<foreignObject>`，不会添加上述 DOM 内容
...
我们提供了[动画](/api/animation/waapi)功能，但一些有认知障碍的用户，会因为动画内容而感到恶心或分心。
