...
在之前的[入门教程](/guide/chapter3)中，我们已经掌握了如何为图形添加事件监听器。在本教程中我们将深入了解监听器被触发时，事件对象上一些有用的属性和方法，同时理解事件传播路径，最终实现一个简单的事件委托效果。

最终示例：

- [官网示例](/examples/event/event-others/#delegate)
...
事件对象上有很多有用的属性，我们先来看看上一节中提到的事件传播路径，通过[composedPath()](/api/event/event-object#composedpath)方法可以获取它。当我们点击 li1 时，此时路径会返回如下结果：
...
- [Canvas](/api/canvas/intro) 即画布对象，可以对应 `window`
- [Document](/api/builtin-objects/document) 文档，可以对应 `window.document`
- [Group(root)](/api/builtin-objects/document#documentelement) 文档根节点，可以对应 `window.document.documentElement`

除了事件传播路径，事件对象上其他的常用属性有：

- [target](/api/event/event-object#target) 返回当前触发事件的图形
- [currentTarget](/api/event/event-object#currenttarget) 总是指向事件绑定的图形
- 各个坐标系下的[事件坐标](/api/event/event-object#canvasxy)
...
更多用法可以参考 [addEventListener()](/api/event/intro#addeventlistener) 的文档。
