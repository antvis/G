...
For this purpose, we provide `g-image-exporter`, which supports functions such as selecting canvas area, exporting dataURL in specified format or saving it as image, [example](/examples/ecosystem/image-exporter/#image-exporter). Some of the functions depend on DOM API, for non-browser environment, please refer to [special platform adaptation of canvas](/api/canvas#special platform adaptation). For example, the download function needs to be implemented by `document.createElement('a')`, non-browser environment needs to pass `document` object by itself.
...
When calling [downloadImage](/guide/advanced-topics/image-exporter#downloadimage) to save and download an image, the value of this configuration item will be used as the default file name if no file name is specified.
...
In this [example](/examples/ecosystem/image-exporter/#image-exporter), we add a background color and watermark that can be drawn by passing in [CanvasRenderingContext2D](<https://developer.mozilla.org/en-US/> docs/Web/API/CanvasRenderingContext2D) can call the Canvas2D API to draw.
...
Note that the crop area uses `Rectangle` instead of [Rect](/api/basic/rect) graphics. Its constructor contains four parameters `x/y/width/height`. It is relative to [viewport coordinate system](/api/canvas/coordinates#viewport) under [viewport](/api/canvas/coordinates#viewport), i.e. for a 400 x 400 canvas, the maximum width and height of the crop is 400.

When exporting [HTML](/api/basic/html), all HTMLElement in the container will be exported by default, but sometimes some elements are not the ones we want to export, so we can use `ignoreElements: (element: Element): boolean;` method to filter. For example, in this [example](/examples/ecosystem/image-exporter/#image-exporter) there are DOM elements added by stats.js and lil-gui in the container that we don't want to export, so we can.
...
Sometimes we want to export vector images. Unlike [toCanvas](/guide/advanced-topics/image-exporter#tocanvas) which is supported for all renderers, only [g-svg](/api/renderer/svg) renderer supports generating SVG type dataURL, if other renderer is selected, `Promise<undefined>` will be returned.
...
To trigger browser download behavior, you can pass [exported dataURL](/examples/ecosystem/image-exporter/#image-exporter) and specify the name of the saved file.
...
In this [example](/examples/ecosystem/image-exporter/#image-exporter), click the button to start downloading the image immediately, and if the `image/png` format is selected, it will eventually be saved as a `my-file.png` file.
...
With [toCanvas](/examples/ecosystem/image-exporter/#image-exporter) we get the HTMLCanvasElement containing the canvas content, using its native method [toDataURL](<https://developer.mozilla>. org/en-cn/docs/Web/API/HTMLCanvasElement/toDataURL) to get the dataURL.
...
Yes, if the canvas contains [HTML](/api/basic/html), the different renderers currently implement the following.
...
In this [example](/examples/ecosystem/image-exporter/#image-exporter), the Tooltip in the top left corner is an HTML.
...
Also when exporting [HTML](/api/basic/html) content, using the export method provided by [html2canvas](https://html2canvas.hertzen.com/) is also an asynchronous operation.

### How do I export graphics outside of the canvas viewport?

The export methods we provide are only for the canvas viewport range, even cropping is relative to [viewport coordinate system](/api/canvas/coordinates#viewport). So if you want to export graphics outside the viewport, you can use [camera API](/api/camera/intro) to change the viewport range without changing the scene structure, for example by [setZoom](http://localhost:8000/en/api/camera#setzoom) to zoom in and out to allow more graphics to fit inside the viewport.
...
In this [example](/examples/ecosystem/image-exporter/#animated-svg), the exported SVG contains the following `<style>` content, using CSS Animations to save some animation effects:
...
But it should be noted that not all properties that support animation can be converted into CSS Animations representation, such as the path property used in [Deformation animation](/api/animation/waapi#Deformation animation). In addition, in [EffectTiming](/api/animation/waapi#effecttiming), some configuration items are not supported by CSS Animations, so they cannot be reflected in the exported file:

- [easing function](/api/animation/waapi#easingfunction)
