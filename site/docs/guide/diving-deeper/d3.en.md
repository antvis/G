...
Now we don't need D3 to create `<svg>`, we just need to give D3 the [root node](/en/api/canvas#document) of the G scene graph, the size of the [canvas](/en/api/canvas) is already specified at creation time.
