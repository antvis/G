...
The G API is as consistent as possible with the DOM API, so some of the DOM API-oriented libraries in the Web ecosystem can be accessed at very low cost, for example [using the Hammer.js gesture library](/en/docs/api/event#directly-using-hammerjs), [using the Interact.js drag-and-drop library](/en/docs/api/event#directly-using-interactjs). For them, G's [event API](/en/docs/api/event) and the DOM Events API are identical:
...
Some stylized rendering plugins can also be used directly, such as [g-plugin-rough-canvas-renderer](/en/docs/plugins/rough-canvas-renderer) for hand-drawn styling of the bar chart above.
...
Now we don't need D3 to create `<svg>`, we just need to give D3 the [root node](/en/docs/api/canvas#document) of the G scene graph, the size of the [canvas](/en/docs/api/canvas) is already specified at creation time.
...
[Example](/en/examples/ecosystem#d3-barchart)
...
In order to get CSS selectors like `g.annotations` to work properly, the [g-plugin-css-selector](/en/docs/plugins/css-select) plugin is required, registered as follows.
...
We can use the G-like DOM API's element query method [querySelectorAll](/en/docs/api/basic/display-object#advanced-query).
...
Finally G needs to avoid the assumption of a "browser real DOM environment" in its own internal implementation, e.g. when [creating canvas](/en/docs/api/canvas#canvas), so that it can run in a WebWorker or even an applet environment.
...
-   [Node & Element API](/en/docs/api/builtin-objects/node) Disguise the G graphic as a real DOM element
-   [Event System](/en/docs/api/event) Provide complete event binding and propagation process
-   [Web Animations API](/en/docs/api/animation/waapi) Provides command-based animation capabilities
-   [CustomElementRegistry](/en/docs/api/canvas#使用-customelementregistry) The graph is created by name, so D3 code like `wrapper.append('g')` actually creates a [Group](/en/docs/api/basic/group) of G
...
This leads us to think about another question, whether G's [animation capabilities](/en/docs/api/animation/waapi) should also be pluggable. In the above example, G only needs to provide rendering capabilities, and the animation of the `d` attribute of `path` is left entirely to D3.
