...
The G API is as consistent as possible with the DOM API, so some of the DOM API-oriented libraries in the Web ecosystem can be accessed at very low cost, for example [using the Hammer.js gesture library](/api/event/gesture-dragndrop#直接使用-interactjs), [using the Interact.js drag-and-drop library](/api/event/gesture-dragndrop#direct-use-of-interactjs). For them, G's [event API](/api/event/intro) and the DOM Events API are identical:
...
Some stylized rendering plugins can also be used directly, such as [g-plugin-rough-canvas-renderer](/plugins/rough-canvas-renderer) for hand-drawn styling of the bar chart above.
...
Now we don't need D3 to create `<svg>`, we just need to give D3 the [root node](/api/builtin-objects/document#documentelement) of the G scene graph, the size of the [canvas](/api/canvas/intro) is already specified at creation time.
...
[Example](/examples/ecosystem/d3/#d3-barchart)
...
In order to get CSS selectors like `g.annotations` to work properly, the [g-plugin-css-selector](/plugins/css-select) plugin is required, registered as follows.
...
In addition, CSS style sheets are often used in D3 projects, such as this [example](/examples/ecosystem/d3/#d3-annotation), which uses `d3-annotation` to set the stroke color.
...
We can use the G-like DOM API's element query method [querySelectorAll](/api/basic/display-object#advanced-query).
...
Finally G needs to avoid the assumption of a "browser real DOM environment" in its own internal implementation, e.g. when [creating canvas](/api/canvas/coordinates#canvas), so that it can run in a WebWorker or even an applet environment.
...
- [Node & Element API](/api/builtin-objects/node) Disguise the G graphic as a real DOM element
- [Event System](/api/event/intro) Provide complete event binding and propagation process
- [Web Animations API](/api/animation/waapi) Provides command-based animation capabilities
- [CustomElementRegistry](/api/canvas/intro#%E4%BD%BF%E7%94%A8-customelementregistry) The graph is created by name, so D3 code like `wrapper.append('g')` actually creates a [Group](/api/basic/group) of G
...
In this [example](/examples/ecosystem/d3/#d3-piechart), we use [d3-shape](https://github.com/d3/d3-shape) and [d3-transition](<https://github.com/d3/> d3-transition) to implement the shape animation.

<img src="https://gw.alipayobjects.com/mdn/rms_6ae20b/afts/img/A*Wq7wSK_0p3AAAAAAAAAAAAAAARQnAQ" width="300px">

This leads us to think about another question, whether G's [animation capabilities](/api/animation/waapi) should also be pluggable. In the above example, G only needs to provide rendering capabilities, and the animation of the `d` attribute of `path` is left entirely to D3.
