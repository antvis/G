...
插件可以极大程度丰富渲染器的能力，在本教程中我们将使用 `g-webgl` 渲染器，配合 [g-plugin-3d](/plugins/3d) 插件渲染一个简单的立方体。更多用法可以参考[插件系统](/plugins/intro)。

其中会涉及以下 API：

- 使用 [registerPlugin](/api/renderer/intro#registerplugin) 为渲染器注册插件
- 使用 [getCamera](/api/canvas/built-in-objects#getcamera) 获取画布相机
- 使用 [setPosition](/api/camera/intro#setpositionx-number--vec3-y-number-z-number) 设置相机位置

最终示例：

- [官网示例](/examples/3d/geometry/#cube)
...
- [g-plugin-3d](/plugins/3d) 提供 3D 渲染能力
- [g-plugin-control](/plugins/control) 提供交互能力
...
然后使用 [setPosition](/api/basic/display-object#平移) 移动它到画布中央：
...
让立方体旋转和其他基础图形一样，使用 [rotate](/api/basic/display-object#旋转) 即可：
