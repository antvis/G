...
G 的 API 和 DOM API 尽可能一致，因此 Web 生态中一些面向 DOM API 的库都可以以非常低的成本接入，例如 [使用 Hammer.js 手势库](/api/event/gesture-dragndrop#直接使用-hammerjs)、[使用 Interact.js 拖拽库](/api/event/gesture-dragndrop#直接使用-interactjs)。对于它们来说，G 的[事件 API](/api/event/intro) 和 DOM Events API 完全一致：
...
在以下[示例](/examples/ecosystem/d3/#d3-barchart)中，我们使用 Fullstack D3 的几个教学例子，在保留绝大部分 D3 风格代码的同时，通过“一行”代码的修改完成渲染 API 的切换，实现 **D3 数据处理 + G 渲染** 的效果。你可以在运行时切换 Canvas、WebGL 和 SVG 的渲染效果：
...
还可以直接使用一些风格化渲染插件，例如通过 [g-plugin-rough-canvas-renderer](/plugins/rough-canvas-renderer) 对上面的柱形图进行手绘风格改造：
...
接下来就是“一行代码”的部分了，现在无需 D3 创建 `<svg>` 了，我们只需要把 G 场景图的[根节点](/api/builtin-objects/document#documentelement)交给 D3，[画布](/api/canvas/intro)的尺寸在创建时就已经指定好了：
...
[完整代码](/examples/ecosystem/d3/#d3-barchart)
...
为了能让 `g.annotations` 这样的 CSS 选择器正常工作，需要使用 [g-plugin-css-selector](/plugins/css-select) 插件，注册方式如下：
...
另外在 D3 项目中经常使用 CSS 样式表，例如该 [示例](/examples/ecosystem/d3/#d3-annotation) 中使用了 `d3-annotation`，设置了描边颜色：
...
我们可以使用 G 类似 DOM API 的元素查询方法 [querySelectorAll](/api/basic/display-object#高级查询)：
...
我们以 D3 为例，[d3-selection](https://github.com/d3/d3-selection) 在创建 DOM 元素时并不会直接使用 `window.document`，而是从元素的 [ownerDocument](/api/builtin-objects/node#ownerdocument) 属性上获取，因此只要 G 的图形上也有该同名属性就可以正常运行，而不会调用到浏览器真实的 DOM API：
...
最后 G 在自身内部实现中，也需要避免“浏览器真实 DOM 环境”这样的假定，例如在[创建画布](/api/canvas/coordinates#canvas)时，这样才能运行在 WebWorker 甚至是小程序环境中。
...
- [Node & Element API](/api/builtin-objects/node)，把 G 的图形“伪装成” 真实的 DOM 元素
- [Event 系统](/api/event/intro)，提供完整的事件绑定以及传播流程
- [Web Animations API 动画系统](/api/animation/waapi)，提供命令式的动画能力
- [CustomElementRegistry](/api/canvas/intro#%E4%BD%BF%E7%94%A8-customelementregistry)，根据名称创建图形，因此 `wrapper.append('g')` 这样的 D3 代码实际上创建了 G 的 [Group]()
...
在该[示例](/examples/ecosystem/d3/#d3-piechart)中，我们使用 [d3-shape](https://github.com/d3/d3-shape) 和 [d3-transition](https://github.com/d3/d3-transition) 实现了形变动画：

<img src="https://gw.alipayobjects.com/mdn/rms_6ae20b/afts/img/A*Wq7wSK_0p3AAAAAAAAAAAAAAARQnAQ" width="300px">

这不禁让我们思考另一个问题，G 的[动画能力](/api/animation/waapi)是否也应该是可插拔的。在上面这个例子中，G 只需要提供渲染能力，对 `path` 的 `d` 属性动画完全交给 D3。
