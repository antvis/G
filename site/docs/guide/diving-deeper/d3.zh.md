...
G 的 API 和 DOM API 尽可能一致，因此 Web 生态中一些面向 DOM API 的库都可以以非常低的成本接入，例如 [使用 Hammer.js 手势库](/zh/api/event#直接使用-hammerjs)、[使用 Interact.js 拖拽库](/zh/api/event#直接使用-interactjs)。对于它们来说，G 的[事件 API](/zh/api/event) 和 DOM Events API 完全一致：
...
接下来就是“一行代码”的部分了，现在无需 D3 创建 `<svg>` 了，我们只需要把 G 场景图的[根节点](/zh/api/canvas#document)交给 D3，[画布](/zh/api/canvas)的尺寸在创建时就已经指定好了：
