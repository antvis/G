...
declare global {
  ...
  interface Uint32Array {
    [Symbol.species]?: 'Uint32Array';
  }
  interface Int8Array {
    [Symbol.species]?: 'Int8Array';
  }
  interface Int16Array {
    [Symbol.species]?: 'Int16Array';
  }
  interface Int32Array {
    [Symbol.species]?: 'Int32Array';
  }
  interface Float32Array {
    [Symbol.species]?: 'Float32Array';
  }
}
...
export default class ArrayBufferSlice {
  constructor(
    ...
    readonly byteLength: number = arrayBuffer.byteLength - byteOffset,
  ) {
    assert(
      byteOffset >= 0 && byteLength >= 0 && byteOffset + byteLength <= this.arrayBuffer.byteLength,
    );
  }

  static fromView(view: ArrayBufferView): ArrayBufferSlice {
   ...
   */
  slice(begin: number, end: number = 0, copyData: boolean = false): ArrayBufferSlice {
    ...
    else return new ArrayBufferSlice(this.arrayBuffer, absBegin, byteLength);
  }
   ...
   * off a small section of a larger buffer, letting the larger buffer get garbage collected.
   */
  subarray(begin: number, byteLength?: number, copyData: boolean = false): ArrayBufferSlice {
    const absBegin = this.byteOffset + begin;
    if (byteLength === undefined) byteLength = this.byteLength - begin;
    assert(byteLength >= 0 && byteLength <= this.byteLength);
    if (copyData)
      return new ArrayBufferSlice(
        ArrayBuffer_slice.call(this.arrayBuffer, absBegin, absBegin + byteLength),
      );
    else return new ArrayBufferSlice(this.arrayBuffer, absBegin, byteLength);
  }
   ...
   */
  copyToBuffer(begin: number = 0, byteLength: number = 0): ArrayBuffer {
  ...
  createDataView(offs: number = 0, length?: number): DataView {
  ...
  convertFromEndianness(endianness: Endianness, componentSize: 1 | 2 | 4): ArrayBufferSlice {
  ...
  createTypedArray<T extends ArrayBufferView>(
    clazz: _TypedArrayConstructor<T>,
    offs: number = 0,
    count?: number,
