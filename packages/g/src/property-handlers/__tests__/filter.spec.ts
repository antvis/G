...
describe('Property Filter', () => {
  it('should parse single filter like blur(5px)', () => {
    expect(parseFilter('blur(5px)', null)).to.be.eqls([
      { name: 'blur', params: [{ unit: 'px', value: 5 }] },
    ]);
  });

  it('should parse multiple filters correctly', () => {
    expect(parseFilter('blur(5px) blur(10px)', null)).to.be.eqls([
      { name: 'blur', params: [{ unit: 'px', value: 5 }] },
  ...
  it('should skip invalid filters', () => {
    expect(parseFilter('blur(5px) xxx() yyy() blur(10px)', null)).to.be.eqls([
      { name: 'blur', params: [{ unit: 'px', value: 5 }] },
      { name: 'blur', params: [{ unit: 'px', value: 10 }] },
    ]);
  });
  ...
  it('should parse single filter like brightness(100%)', () => {
    expect(parseFilter('brightness(100%)', null)).to.be.eqls([
      { name: 'brightness', params: [{ unit: '%', value: 100 }] },
    ]);

    expect(parseFilter('brightness(0.5)', null)).to.be.eqls([
      { name: 'brightness', params: [{ unit: '', value: 0.5 }] },
    ]);
  });

  it('should parse single filter like drop-shadow', () => {
    expect(parseFilter('drop-shadow(16px 16px 10px black)', null)).to.be.eqls([
      {
        name: 'drop-shadow',
        params: [
          { unit: 'px', value: 16 },
    ...
    expect(
      parseFilter('drop-shadow(0.232715008431704px 6.924114671163576px 0px #000)', null),
    ).to.be.eqls([
      {
        name: 'drop-shadow',
        params: [
