import { toRadian, mod } from '@antv/util';
import { isSamePoint } from './math';
...
// u.v/|u||v|，计算夹角的余弦值
function vRatio(u: [number, number], v: [number, number]) {
  // 当存在一个向量的长度为 0 时，夹角也为 0，即夹角的余弦值为 1
  return vMag(u) * vMag(v) ? (u[0] * v[0] + u[1] * v[1]) / (vMag(u) * vMag(v)) : 1;
}

// 向量角度
function vAngle(u: [number, number], v: [number, number]) {
...
export default function getArcParams(startPoint: [number, number], params, x: number, y: number): ArcParams {
  let rx = params[1];
  let ry = params[2];
