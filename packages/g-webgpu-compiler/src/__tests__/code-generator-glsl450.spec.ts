...
describe('Code Generator', () => {
  ...
  const generator = new CodeGeneratorGLSL450();
...
#define BOOL2 true`);
      ...
      test('should generate uniform & buffer declaration statement correctly.', () => {
        compiler.setTarget(Target.GLSL450);
...
return 2;}`);
    ...
    describe('Demos', () => {
      ...
      class Add2Vectors {
        ...
        expect(code).toContain(`layout(std430, set = 0, binding = 0) buffer   GWebGPUBuffer0 {
  float vectorA[];
} gWebGPUBuffer0;
...
layout(std430, set = 0, binding = 1) buffer readonly  GWebGPUBuffer1 {
  float vectorB[];
        ...
        expect(code).toContain(`float sum(float a, float b) {return a + b;}
