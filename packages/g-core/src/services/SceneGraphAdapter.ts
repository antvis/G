 ...
 */
@injectable()
export class SceneGraphAdapter implements Adapter<Group, Group> {
  @inject(GroupPool)
  protected groupPool: GroupPool;

  isTag(node: Group): node is Group {
    return true;
  }

  existsOne(test: Predicate<Group>, nodes: Group[]): boolean {
  ...
  getAttributeValue(node: Group, name: string): string | undefined {
  ...
  getText(node: Group): string {
    return '';
  }

  hasAttrib(node: Group, name: string) {
    return !!node.getEntity().getComponent(SceneGraphNode).attributes[name];
  }

  removeSubsets(nodes: Group[]): Group[] {
  ...
  findAll(test: Predicate<Group>, nodes: Group[]): Group[] {
    let result: Group[] = [];
  ...
  findOne(test: Predicate<Group>, nodes: Group[]): Group | null {
    let node = null;
