...
export default {
  ...
  length(x1, y1, x2, y2, x3, y3, x4, y4) {
    // 迭代三次，划分成 8 段求长度
    return cubicLength(x1, y1, x2, y2, x3, y3, x4, y4, 3);
  },
  nearestPoint(x1, y1, x2, y2, x3, y3, x4, y4, x0, y0) {
    return nearestPoint([x1, x2, x3, x4], [y1, y2, y3, y4], x0, y0, cubicAt);
  },
  pointDistance(x1, y1, x2, y2, x3, y3, x4, y4, x0, y0) {
    const point = this.nearestPoint(x1, y1, x2, y2, x3, y3, x4, y4, x0, y0);
    return distance(point.x, point.y, x0, y0);
  },
  interpolationAt: cubicAt,
  pointAt(x1, y1, x2, y2, x3, y3, x4, y4, t) {
    return {
      x: cubicAt(x1, x2, x3, x4, t),
      y: cubicAt(y1, y2, y3, y4, t),
    };
  },
  divide(x1, y1, x2, y2, x3, y3, x4, y4, t) {
    return divideCubic(x1, y1, x2, y2, x3, y3, x4, y4, t);
  },
  tangentAngle(x1, y1, x2, y2, x3, y3, x4, y4, t) {
