...
import { Entity, Matcher } from '@antv/g-ecs';
...
export interface IRenderPass<RenderPassData> {
   ...
   * 调用渲染引擎服务完成虚拟资源的实例化
   */
  execute(fg: FrameGraphEngine, pass: FrameGraphPass<RenderPassData>, entities: Entity[]): Promise<void>;
...
export class FrameGraphEngine {
  ...
  public addPass<PassData>(
    ...
    execute: (fg: FrameGraphEngine, pass: FrameGraphPass<PassData>, entities: Entity[]) => Promise<void>,
    tearDown?: () => void
  ) {
