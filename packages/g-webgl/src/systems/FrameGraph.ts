...
export interface IRenderPass<RenderPassData> {
   ...
   * 调用渲染引擎服务完成虚拟资源的实例化
   */
  execute(fg: FrameGraphSystem, pass: FrameGraphPass<RenderPassData>, entities: Entity[]): Promise<void>;
...
export class FrameGraphSystem extends System {
  ...
  public addPass<PassData>(
    name: string,
    setup: (fg: FrameGraphSystem, passNode: PassNode, pass: FrameGraphPass<PassData>) => void,
    execute: (fg: FrameGraphSystem, pass: FrameGraphPass<PassData>, entities: Entity[]) => Promise<void>,
    tearDown?: () => void
  ) {
  ...
  public async executePassNodes(entities: Entity[]) {
    for (const [index, node] of this.passNodes.entries()) {
      if (node.refCount) {
        ...
        await this.frameGraphPasses[index].execute(this, this.frameGraphPasses[index], entities);
