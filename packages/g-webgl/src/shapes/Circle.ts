import { SceneGraphNode, SHAPE } from '@antv/g-core';
...
export class CircleRenderer extends BaseRenderer {
  ...
  async prepareModel(context: RenderingContext, entity: Entity) {
    ...
    material.setUniform({
      ...
      [UNIFORM.Shape]: pointShapes.indexOf(sceneGraphNode.tagName),
      ...
      [UNIFORM.Anchor]: sceneGraphNode.tagName === SHAPE.Rect ? [1, 1] : [0, 0],
    });

    let config: Partial<IPointConfig>[] = [];
    if (instancing) {
      config = renderable3d.instanceEntities.map((subEntity) => {
        const [halfWidth, halfHeight] = this.getSize(subEntity.getComponent(SceneGraphNode));
        return {
      ...
      const [halfWidth, halfHeight] = this.getSize(sceneGraphNode);
      config.push({
        size: [halfWidth - lineWidth / 2, halfHeight - lineWidth / 2],
        color: fillColor as [number, number, number, number], // sRGB
        opacity: fillOpacity,
  ...
  private getSize(sceneGraphNode: SceneGraphNode) {
    const { attributes, tagName } = sceneGraphNode;
