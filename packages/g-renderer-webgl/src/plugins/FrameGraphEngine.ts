...
export interface IRenderPass<RenderPassData> {
   ...
   * 调用渲染引擎服务完成虚拟资源的实例化
   */
  execute(fg: FrameGraphEngine, pass: FrameGraphPass<RenderPassData>, entities: Entity[]): void;
...
export class FrameGraphEngine {
  ...
  public addPass<PassData>(
    ...
    execute: (fg: FrameGraphEngine, pass: FrameGraphPass<PassData>, entities: Entity[]) => void,
    tearDown?: () => void
  ) {
  ...
  public executePassNodes(entities: Entity[]) {
    for (let index = 0; index < this.passNodes.length; index++) {
      ...
      if (node.refCount) {
        ...
        this.frameGraphPasses[index].execute(this, this.frameGraphPasses[index], entities);
