import { Renderable, SceneGraphNode } from '@antv/g';
...
export class ImageModelBuilder implements ModelBuilder {
  ...
  async onAttributeChanged(entity: Entity, name: string, value: any) {
    ...
    if (renderable3d.sourceEntity) {
      ...
      const { img, width = 0, height = 0 } = sceneGraphNode.attributes;
      if (name === 'img') {
  ...
  async prepareModel(context: RenderingContext, entity: Entity) {
    ...
    const { width = 0, height = 0, img, anchor = [0, 0] } = sceneGraphNode.attributes;
