...
G 的默认相机设置如下，其中 `width/height` 为 [Canvas](/en/docs/api/canvas) 的尺寸，[使用示例](/en/examples/camera#ortho)：
...
[使用示例](/en/examples/camera#perspective)：
...
在 G 内置的正交投影相机中，默认设置为 `[width / 2, height / 2, 500]`，其中 `width/height` 为 [Canvas](/en/docs/api/canvas) 的尺寸。因此如果我们想重新设置相机的 `x/y` 坐标，同时保持 `z` 坐标不变，可以这么做：
...
在 G 内置的正交投影相机中，默认设置为 `[width / 2, height / 2, 0]`，其中 `width/height` 为 [Canvas](/en/docs/api/canvas) 的尺寸。因此如果我们想重新设置相机视点的 `x/y` 坐标，同时保持 `z` 坐标不变，可以这么做：
...
`zoom` 大于 1 代表放大，反之代表缩小，[示例](/en/examples/camera#ortho)。
...
仅透视投影下生效，视角越大容纳的对象越多。[示例](/en/examples/camera#perspective)
...
设置最小视距。在进行 [dolly](/en/docs/api/camera#dolly) 操作时不会小于该距离。

默认值为 `-Infinity`。

### setMaxDistance()

设置最大视距。在进行 [dolly](/en/docs/api/camera#dolly) 操作时不会大于该距离。
...
在该[示例](/en/examples/camera#view-offset)中，[Cube](/en/docs/api/3d/geometry#cubegeometry) 原本位于视口正中央，通过设置 `x/y` 偏移量到视口中心。

<img src="https://gw.alipayobjects.com/mdn/rms_6ae20b/afts/img/A*U6ELSY2EVNIAAAAAAAAAAAAAARQnAQ" alt="setViewOffset" width="300">

在 [g-plugin-device-renderer](/en/docs/plugins/device-renderer) 中拾取时，我们使用该方法设置偏移量（将相机对准拾取区域），仅渲染拾取区域而非整个屏幕以提高性能。

### clearViewOffset()

清除之前设置的视口偏移量，立刻重新计算投影矩阵。

在该[示例](/en/examples/camera#view-offset)中，点击按钮可以随时移除已设置的偏移量。
...
注意不同的[相机类型](/en/docs/api/camera#相机类型)下，固定相机位置和固定视点位置旋转的效果不同：
...
配合 [g-plugin-control](/en/docs/plugins/control) 可以使用鼠标平移、缩放进行交互，[示例](/en/examples/camera#landmark)。

### Orbiting

固定视点 `focalPoint`，改变相机位置 `position`。常用于 CAD 观察模型这样的场景，但不能跨越南北极。

在 Three.js 中称作 [OrbitControls](https://threejs.org/docs/#examples/en/controls/OrbitControls)

在该[示例](/en/examples/camera#landmark)中，我们通过鼠标的平移控制相机完成 [pan](/en/docs/api/camera#pan) 动作，仿佛是在让场景绕固定视点“旋转”。
...
在该[示例](/en/examples/camera#landmark)中，我们通过鼠标的平移控制相机完成 [pan]() 动作，让相机绕固定视点“旋转”。
...
很自然的，根据相机类型的不同，同一个摄像机动作对应的实现也不同。 我们以 [dolly]() 动作为例，同样都是一个向前向后移动摄像机位置的动作，对于 [Orbiting](/en/docs/api/camera#orbiting) / [Exploring](/en/docs/api/camera#exploring) 模式视点不变，而在 [Tracking](/en/docs/api/camera#tracking) 模式下视点是需要调整的。
...
在该[示例](/en/examples/camera#action) 中，下面的操作将导致原本处于视点处的物体展示在左上角：

```ts
camera.pan(200, 200);
```

在 [g-plugin-control](/en/docs/plugins/control) 中我们响应鼠标平移事件，调用该方法：

<img src="https://gw.alipayobjects.com/mdn/rms_6ae20b/afts/img/A*QjQQRLA3w8sAAAAAAAAAAAAAARQnAQ">

### dolly()

沿 n 轴移动相机。固定视点，改变相机位置从而改变视距。会保持视距在 [minDistance](/en/docs/api/camera#setmindistance) 和 [maxDistance](/en/docs/api/camera#setmaxdistance) 之间。
...
在透视投影中效果如下，在 [g-plugin-control](/en/docs/plugins/control) 中我们响应鼠标滚轮事件，调用该方法：
...
我们可以把相机当前的位置、视点记录下来，保存成一个"地标" Landmark。随后当相机参数发生改变时，可以随时切换到之前保存的任意一个 Landmark，同时带有平滑的切换动画，类似真实片场中的摄像机摇臂，在一些应用中也称作 `flyTo`（例如 [Mapbox 中的应用](https://docs.mapbox.com/mapbox-gl-js/example/flyto/)），[示例](/en/examples/camera#landmark)。
...
-   markName 名称
-   options 相机参数，包括：
    -   position 世界坐标系下的相机位置，取值类型参考 [setPosition](/en/docs/api/camera#setposition)
    -   focalPoint 世界坐标系下的视点，取值类型参考 [setFocalPoint](/en/docs/api/camera#setfocalpoint)
    -   roll 旋转角度，取值类型参考 [setRoll](/en/docs/api/camera#setroll)
    -   zoom 缩放比例，取值类型参考 [setZoom](/en/docs/api/camera#setzoom)
...
切换到之前保存的 Landmark，在 2D 和 3D 场景下都适用，[示例](/en/examples/camera#landmark2)：
...
-   options 动画参数，包括：
    ...
    -   easing 缓动函数，默认值为 `linear`。和动画系统一致的[内置效果](/en/docs/api/animation#easing-1)
    -   easingFunction 自定义缓动函数，当内置的缓动函数无法满足要求时，可以[自定义](/en/docs/api/animation#easingfunction)
    -   onfinish 动画结束后的回调函数

和动画系统中的 [options](/en/docs/api/animation#options) 参数一样，传入 `number` 时等同于设置 `duration`：
