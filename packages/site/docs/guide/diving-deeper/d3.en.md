...
-   [Web Animation API](/en/docs/api/animation) Provides command-based animation capabilities
...
This leads us to think about another question, whether G's [animation capabilities](/en/docs/api/animation) should also be pluggable. In the above example, G only needs to provide rendering capabilities, and the animation of the `d` attribute of `path` is left entirely to D3.
