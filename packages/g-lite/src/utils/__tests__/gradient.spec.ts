...
describe('Gradient parser', () => {
  it('should parse linear-gradient correctly', () => {
    ...
    expect(ast).to.be.eqls([
      {
        colorStops: [
          ...
          { length: { type: '%', value: '40' }, type: 'literal', value: 'green' },
    ...
    // multiple gradients
    ast = parseGradient(`linear-gradient(217deg, rgba(255,0,0,.8), rgba(255,0,0,0) 70.71%),
    ...
    expect(ast).to.be.eqls([
      {
        colorStops: [
          ...
          { length: { type: '%', value: '70.71' }, type: 'rgba', value: ['255', '0', '0', '0'] },
        ],
      ...
      },
      {
        colorStops: [
          ...
          { length: { type: '%', value: '70.71' }, type: 'rgba', value: ['0', '255', '0', '0'] },
        ],
      ...
      },
      {
        colorStops: [
          ...
          { length: { type: '%', value: '70.71' }, type: 'rgba', value: ['0', '0', '255', '0'] },
        ],
  ...
  it('should parse radial-gradient correctly', () => {
    let ast = parseGradient('radial-gradient(circle at center, red, blue, green 100%)');
    expect(ast).to.be.eqls([
      {
        colorStops: [
          { length: undefined, type: 'literal', value: 'red' },
          { length: undefined, type: 'literal', value: 'blue' },
          { length: { type: '%', value: '100' }, type: 'literal', value: 'green' },
        ],
        orientation: [
          {
            at: {
              type: 'position',
              value: { x: { type: 'position-keyword', value: 'center' }, y: undefined },
            },
            style: undefined,
            type: 'shape',
            value: 'circle',
    ...
    ast = parseGradient('radial-gradient(red, blue, green), radial-gradient(red, blue, green)');
    expect(ast).to.be.eqls([
      {
        colorStops: [
          { length: undefined, type: 'literal', value: 'red' },
