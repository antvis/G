...
describe('svg shape diff efficiency', () => {
  ...
  it('attr diff', () => {
    ...
    canvas.draw();
    expect(group.cfg.el.getAttribute('transform')).to.equal('matrix(1,0,0,1,100,100)');
    expect(shape.attrs.fill).to.equal('red');
    expect(shape.cfg.el.getAttribute('fill')).to.equal('blue');
  });
});
