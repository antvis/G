...
describe('SVG Canvas', () => {
  ...
  describe('Canvas 容器操作', () => {
    it('new canvas', () => {
      ...
      const matrix = canvas.attrs.matrix;
  ...
  describe('canvas 事件', () => {
    ...
    it('toFront', () => {
      ...
      circle.toFront();
      canvas.draw();
      const children = canvas.cfg.children;
      expect(children[children.length - 1].attr('id')).to.equal(circle.attrs.id);
    });
    it('toBack', () => {
      circle.toBack();
      canvas.draw();
      const children = canvas.cfg.children;
      expect(children[0].attr('id')).to.equal(circle.attrs.id);
    });
    it('zIndex', () => {
      circle.setZIndex(5);
      canvas.draw();
    ...
    xit('getClientByPoint', () => {
      const pixelRatio = canvas.get('pixelRatio');
      const bbox = canvas.getClientByPoint(100, 100);
      expect(bbox.clientX).to.equal(100 / pixelRatio);
    });
  });
});
