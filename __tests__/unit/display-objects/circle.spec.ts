...
describe('Circle', () => {
  it("should calc Circle's GeometryBounds, RenderBounds, Bounds and LocalBounds correctly", () => {
    ...
    expect(circle.getLocalPosition()).eqls([200, 100, 0]);
    bounds = circle.getBounds();
    if (bounds) {
      expect(bounds.center).eqls([210, 110, 0]);
      expect(bounds.halfExtents).eqls([10, 10, 0]);
    }
    ...
    bounds = circle.getBounds();
    if (bounds) {
      expect(bounds.center).eqls([190, 90, 0]);
      expect(bounds.halfExtents).eqls([10, 10, 0]);
    }
    geometryBounds = circle.getGeometryBounds();
    if (geometryBounds) {
  ...
  it("should reset Circle's default values correctly", () => {
    ...
    expect(circle.parsedStyle.transformOrigin).to.be.eqls([
      CSS.percent(50),
      CSS.percent(50),
    ]);
