...
describe('DisplayObject Node API', () => {
  ...
  it('should (deep) cloneNode correctly', () => {
    ...
    expect((cloned.children[0] as DisplayObject).getPosition()).to.be.eqls(
      group2.getPosition(),
    );
    expect(cloned.children[0].style.fontSize).to.be.eqls(group2.style.fontSize);
    expect(cloned.children[0].style.transform).to.be.eqls(
  ...
  it('should (deep) cloneNode for custom elements correctly', async () => {
    ...
    expect((cloned.childNodes[0] as Circle).className).to.be.eqls(
      'testclassname',
    );
