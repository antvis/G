...
describe('Image', function() {
  ...
  it('init attr', function() {
    ...
    expect(image.attr('img')).to.be.null;
    expect(image.attr('width')).to.equal(0);
    expect(image.attr('height')).to.equal(0);
    expect(image.attr('sx')).to.be.null;
    expect(image.attr('sy')).to.be.null;
    expect(image.attr('swidth')).to.be.null;
    expect(image.attr('sheight')).to.be.null;
  ...
  it('normal use', function() {
    const image1 = new G.Image({
      attrs: {
        ...
        img: resolve(process.cwd(), './__tests__/fixtures/test1.jpg'), // relative to test/unit
      },
    });
