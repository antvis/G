...
describe('Text', function() {
  ...
  it('init attrs', function() {
    ...
    expect(text.getBBox().equals(new BBox(0, 0, 0, 0))).to.be.true;
  });
  ...
  it('x', function() {
    ...
    box = text1.getBBox();
    expect(box.equals(new BBox(10, 0, 0, 0))).to.be.true;
  ...
  it('y', function() {
    ...
    box = text1.getBBox();
    expect(box.equals(new BBox(0, 20, 0, 0))).to.be.true;
    text1.attr({
      x: 0,
      y: 20,
      text: '你好啊',
  ...
  it('textAlign', function() {
    ...
    expect(text1.getBBox().equals(new BBox(0, 0, 0, 0))).to.be.true;
...
describe('Text 不存在', function() {
  ...
  it('text 空 "" ', function() {
    ...
    expect(text.attr('textBaseline')).to.equal('top');
    expect(text.attr('lineWidth')).to.equal(1);
    expect(text.getBBox().equals(new BBox(50, 50, 0, 0))).to.be.true;
    canvas.add(rect);
    canvas.add(text);
    canvas.draw();
  });
  ...
  it('modify multi-lines text', (done) => {
    ...
    canvas.draw();
    setTimeout(() => {
      expect(text.attrs.textArr.length).to.equal(2);
      text.attr('text', 'line');
      canvas.draw();
